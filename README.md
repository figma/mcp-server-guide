...
This repo also contains standalone workflow skills that are not bundled with the Figma plugin. See [`workflow-skills/video-interaction-mapper`](workflow-skills/video-interaction-mapper) for a community skill that turns UI recordings into annotated Figma storyboards.
