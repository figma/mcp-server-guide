...
**MANDATORY**: You MUST also load figma-use (load `readPowerSteering("figma", "figma-use.md")`) before any `use_figma` call. That skill contains critical rules (color ranges, font loading, etc.) that apply to every script you write.
...
- If the user wants to generate **code from a Figma design**, switch to [figma-implement-design](#build--update-screens-and-views-from-design-system).
- If the user wants to create **new reusable components or variants**, use figma-use (load `readPowerSteering("figma", "figma-use.md")`) directly.
- If the user wants to write **Code Connect mappings**, switch to figma-code-connect (load `readPowerSteering("figma", "code-connect-components.md")`).
...
See figma-use (load `readPowerSteering("figma", "figma-use.md")`) for binding patterns.
...
See figma-use (load `readPowerSteering("figma", "figma-use.md")`) and figma-use (load `readPowerSteering("figma", "figma-use.md")`) for details.

### Step 3: Create the Wrapper Frame First With Placeholders

**MUST use $fig** Use `$fig` for all node creation. See figma-use (load `readPowerSteering("figma", "figma-use.md")`) for API reference.
...
Component instances ship with placeholder text ("Title", "Heading", "Button"). Use the component property keys you discovered in Step 2 to override them with `setProperties()` — this is more reliable than direct `node.characters` manipulation. See figma-use (load `readPowerSteering("figma", "figma-use.md")`) for the full pattern.
...
For detailed API patterns and gotchas, load these from the figma-use (load `readPowerSteering("figma", "figma-use.md")`) references as needed:

- figma-use (load `readPowerSteering("figma", "figma-use.md")`) — importing by key, finding variants, setProperties, text overrides, working with instances
- figma-use (load `readPowerSteering("figma", "figma-use.md")`) — creating/binding variables, importing library variables, scopes, aliasing, discovering existing variables
- figma-use (load `readPowerSteering("figma", "figma-use.md")`) — creating/applying text styles, importing library text styles, type ramps
- figma-use (load `readPowerSteering("figma", "figma-use.md")`) — creating/applying effect styles (shadows), importing library effect styles
- figma-use (load `readPowerSteering("figma", "figma-use.md")`) — layout pitfalls (HUG/FILL interactions, counterAxisAlignItems, sizing order), paint/color issues, page context resets

## Error Recovery

Follow the error recovery process from figma-use (load `readPowerSteering("figma", "figma-use.md")`):
