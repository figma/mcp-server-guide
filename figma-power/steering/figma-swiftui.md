...
|---|---|---|
| **Design → code** | User wants SwiftUI in their iOS project from a Figma file/frame | [Figma → SwiftUI (design → code)](#reference--figma--swiftui-design--code) |
| **Code → design** | User wants to push SwiftUI views / screens / tokens into a Figma file | [SwiftUI → Figma (code → design)](#reference--swiftui--figma-code--design) |
...
6. **For code → design, `use_figma` is the API.** Always load `figma-use` (load `readPowerSteering("figma", "figma-use.md")`) before any `use_figma` call. If the task involves building a full screen, also load `figma-generate-design` (load `readPowerSteering("figma", "figma-generate-design.md")`); if it involves building components or a design system, also load `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`).

## References

| Doc | When to load |
|---|---|
| [Figma → SwiftUI (design → code)](#reference--figma--swiftui-design--code) | Translating a Figma design / frame into SwiftUI |
| [SwiftUI → Figma (code → design)](#reference--swiftui--figma-code--design) | Pushing SwiftUI views / screens / tokens into Figma |

---

## Reference — Figma → SwiftUI (design → code)

Translate a Figma design into idiomatic SwiftUI. Assumes the shared context in [this skill](#figma--swiftui) is loaded (URL parsing, `clientLanguages`/`clientFrameworks`, "React+Tailwind is a reference not a source", screenshot is truth).
...
Call `get_design_context` with `nodeId` and `fileKey` extracted from the Figma URL (see [this skill](#figma--swiftui) shared context #1), passing:
...
## Reference — SwiftUI → Figma (code → design)

Translate SwiftUI source code into a Figma design using `use_figma`. This doc covers the SwiftUI-specific translation knowledge; the API-level rules for `use_figma` itself live in `figma-use` (load `readPowerSteering("figma", "figma-use.md")`), the screen-assembly workflow lives in `figma-generate-design` (load `readPowerSteering("figma", "figma-generate-design.md")`), and the component / variant / variable workflow lives in `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`). Load whichever of those match the scope of the request, and use this doc to drive the *SwiftUI semantic interpretation* of the source.

Assumes the shared context in [this skill](#figma--swiftui) is loaded.
...
|---|---|---|
| A single view / component (one `Button` style, one row, one card) | A single component or component set | `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`) |
| A screen — a `NavigationStack { … }`, a `TabView { … }` body, or a `View` that renders a whole route | A frame on a page, composed from design-system components | `figma-generate-design` (load `readPowerSteering("figma", "figma-generate-design.md")`) |
| A multi-screen flow — every screen in an app, plus shared tokens | A design system + screen frames | `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`), then `figma-generate-design` (load `readPowerSteering("figma", "figma-generate-design.md")`) |
| A standalone token set (colors, typography, spacing) with no views | Variable collections only | `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`) |
| A target Figma file does not yet exist | Create one first | `figma-create-new-file` (load `readPowerSteering("figma", "figma-create-new-file.md")`) before any `use_figma` call |
...
Before writing any `use_figma` script, run the discovery the `figma-use` (load `readPowerSteering("figma", "figma-use.md")`) and `figma-generate-design` (load `readPowerSteering("figma", "figma-generate-design.md")`) skills already require — but with SwiftUI-shaped expectations:
...
- **Code Connect mappings.** If the project already has `figma-code-connect` templates pointing the existing Figma components at SwiftUI source (or vice versa), follow them — don't generate parallel components. See `figma-code-connect` (load `readPowerSteering("figma", "code-connect-components.md")`).
...
| `VStack`, `HStack` | `figma.createAutoLayout('VERTICAL')` / `figma.createAutoLayout('HORIZONTAL')` — never absolute `x`/`y` for arranging contents. See `figma-use` (load `readPowerSteering("figma", "figma-use.md")`) Rule 12a. |
...
If the source uses a project-specific token (`Color.brandPrimary`, `Color("AccentColor")`), look for the matching semantic variable in Figma before creating a new one. When creating variables, follow `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`) — set scopes explicitly (`FRAME_FILL, SHAPE_FILL` for backgrounds, `TEXT_FILL` for labels, `STROKE_COLOR` for separators); never leave `ALL_SCOPES`.
...
For long-term linkage, set up a Code Connect mapping pointing a Figma "Icon" component (with an `INSTANCE_SWAP` property for each symbol) at the SwiftUI source's `Image(systemName: …)` — see `figma-code-connect` (load `readPowerSteering("figma", "code-connect-components.md")`). The next design → code task on this file will then return SwiftUI directly.
...
Once the Figma component exists and the SwiftUI source is in the repo, set up a Code Connect mapping so the design and the code stay linked. See `figma-code-connect` (load `readPowerSteering("figma", "code-connect-components.md")`) for the workflow. The mapping is what makes the *next* design → code task on the same file return correct SwiftUI snippets instead of generic React+Tailwind.
...
Per `figma-use` (load `readPowerSteering("figma", "figma-use.md")`), every `use_figma` script must `return` all created/mutated node IDs and follow the incremental workflow (skeleton first, then fill, validate with `get_metadata` / screenshots between steps). For SwiftUI → Figma specifically:
...
4. **One screen at a time.** Even for multi-screen flows, build one screen, take a screenshot, get sign-off, then move on. The same checkpoint discipline from `figma-generate-design` (load `readPowerSteering("figma", "figma-generate-design.md")`) and `figma-generate-library` (load `readPowerSteering("figma", "figma-generate-library.md")`) applies.
